...
class CancelOrderServiceTest {
    ...
    @Test
    void shouldCancelOrderWhenFoundAndPending() {
        ...
        Optional<OrderResult> result = cancelOrderService.cancelOrder(orderId);

        assertTrue(result.isPresent());
        assertEquals(orderId.getValue(), result.get().orderId().getValue());
        assertEquals(OrderStatus.CANCELLED.name(), result.get().status());
